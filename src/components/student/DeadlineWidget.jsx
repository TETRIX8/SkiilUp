// src/components/student/DeadlineWidget.jsx
import React, { useEffect, useMemo, useRef } from 'react';
import './deadline-loading.css';

function daysBetweenNowAnd(dateStr) {
  if (!dateStr) return 7;
  const now = new Date();
...
export default function DeadlineWidget({ dueDate, durationSec = 20 }) {
  const days = useMemo(() => daysBetweenNowAnd(dueDate), [dueDate]);
  const dayRef = useRef(null);
  const daysWordRef = useRef(null);

  useEffect(() => {
    const progress = document.getElementById('progress-time-fill');
    const death = document.getElementById('death-group');
    if (progress) progress.style.animationDuration = `${durationSec}s`;
    if (death) death.style.animationDuration = `${durationSec}s`;

    function updateLabel(value) {
      if (dayRef.current) dayRef.current.textContent = String(value);
      if (daysWordRef.current) daysWordRef.current.textContent = pluralizeRuDays(value);
    }
    ...
    function runCycle() {
      let current = days;
      updateLabel(current);
      const stepMs = (durationSec * 1000) / days;
    ...
    // mimic arm speed-ups over time, scoped by id in this component subtree
    const arm = document.getElementById('designer-arm-grop');
    const timeouts = [];
    if (arm) {
      arm.style.animationDuration = '1.5s';
      timeouts.push(setTimeout(() => (arm.style.animationDuration = '1s'), 4000));
      timeouts.push(setTimeout(() => (arm.style.animationDuration = '0.7s'), 8000));
      timeouts.push(setTimeout(() => (arm.style.animationDuration = '0.3s'), 12000));
      timeouts.push(setTimeout(() => (arm.style.animationDuration = '0.2s'), 15000));
    }

    let intervalId = runCycle();
  ...
  return (
    <div className="deadline-widget-container">
      <div id="deadline">
        <svg preserveAspectRatio="none" id="line" viewBox="0 0 581 158">
          ...
          <g>
            ...
            <rect id="progress-time-fill" x="-100%" y="34" clipPath="url(#SVGID_2_)" fill="#BE002A" width="586" height="103"/>
          </g>
          <g id="death-group">
            ...
            <path id="death-tool" fill="#BE002A" d="M-20.996,26.839l-42.819,91.475l1.812,0.848l38.342-81.909c0,0,8.833,2.643,12.412,7.414 c5,6.668,4.75,14.084,4.75,14.084s4.354-7.732,0.083-17.666C-10,32.75-19.647,28.676-19.647,28.676l0.463-0.988L-20.996,26.839z"/>
          </g>
        ...
        <div className="deadline-days">
          Deadline <span ref={dayRef} className="day">{days}</span> <span ref={daysWordRef} className="days">{pluralizeRuDays(days)}</span>
        </div>
      </div>
    </div>
  );
